...
And then additional FHIR Resources as part of the package.

NOTE: This is compatible with the generated `./output/package.tgz` generated by IGPublisher
...
{MULTIPLE-VERSION-PACKAGE-MANIFEST}
```

Where `MULTIPLE-VERSION-PACKAGE-MANIFEST` is the NPM form of a multiple-version package manifest:
```jsonc
{
    "_id": "package.id",
    "name": "package.id",
...
NOTE: It might be convenient to extract, decode, and store the actual tarball data in the attachment in a blob store or file system. And a server MAY rewrite the tarball location in the versions section to match implementation details.

If the version of the package already exists, the server returns error `422`, otherwise the server returns `201`.
...
To implement downloading a NPM FHIR Package, the knowledge artifact repository must implement 3 endpoints:

* `GET http://registry/{packageId}` => Multiple-version Package Manifest
